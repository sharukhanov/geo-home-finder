...
Адрес `*.up.railway.app` из России без VPN не открывается — соединение
обрывается на уровне сети. Варианты, от дешёвого к надёжному:

1. **Свой домен на том же Railway.** Купить домен (например `.ru`) и привязать
   его в **Settings → Networking → Custom Domain**. Помогает, если блокируется
   имя `railway.app`, а не сами адреса серверов. Проверяется за вечер и домен
   всё равно понадобится.
2. **Переезд на хостинг, доступный из России** (Timeweb Cloud, Selectel,
